...
- STATUS_AS_OF:2026-08-05(更新者:Claude 主 harness,經 obsidian-vault-manager;本次為 08-05 當天第二次更新——上一版仍是早上 09:40 啟動的 schema 共識案「尚未收斂」的快照,現已推進到 P4 操作點實驗開跑+codex 審完)
...
- **執行中程序**:**無訓練在跑,但有一個推論 job 在背景執行**。09:40 啟動的 mask/prediction-artifact JSON 序列化 schema 共識案已收斂並產出格式,**P4 操作點（決策閾值）實驗已開跑,但 prereg 尚未凍結**：背景 job `analysis/p4_operating_point/dump_prob.py --all`（repo `/mnt/backup/alanyh/oil_IR_Fullband/OIL_PROJECT_MutiBand_GT_expand`）。**是推論不是訓練**——用正典 `_dlv3_det` 三折既有 `best.pt` 重跑 GT-aware reconstruction,依 2026-08-05 prediction-artifact 共識產出 JSON hard mask + probability NPZ；約 4 小時,log 在 `analysis/p4_operating_point/dump_prob.log`。**它是自己的 session leader,不會被 /clear 或 claude 進程結束影響**。接手時先看 log 尾巴，並用場景數確認完整度:`analysis/p4_operating_point/recon/fold{1,2,3}/gt_aware_recon/prediction_artifacts/` 下 `.pred.json` 檔數，每折應為 119/118/118 景。
  - **codex 已審過 P4 prereg draft,VERDICT: REVISE,8 條 BLOCKING**（全文含統計方法修正一律見 institution 正本 `research/oil_spill_project_status.md` 第 9 條,這裡只列最關鍵三條）:①fold-level leave-one-fold-out 選閾值在僅 12 事件群語料下構成實質洩漏,須改事件層 cross-fitting 且 bootstrap 要重跑完整選取程序;②「τ=0.5 降低後 IoU 不升⇒沒有機率質量」是邏輯錯誤（至少 8 種替代解釋）,須改用 GT-positive 像素 p<0.1/0.1≤p<0.5/p≥0.5 三段分佈+per-scene oracle threshold IoU;③事後依 oil_px 選出的 top-10 巨型場景不得當正式機制端點,降為凍結的次要 case series,正式分層改用既有 large/M_L。
  - **🔴 卡住的決策,只有使用者能答,擋住 prereg 凍結**:**計畫任務最終以 pooled 還是 Contract v1.0 的 S 評成敗？** 現況是招牌數字用 pooled 0.4399、採用門檻用 S,若 P4 使 S↑而 pooled↓即語意矛盾,不得一邊用 S 採用一邊仍把 0.4399 當唯一好壞語言。未答之前 codex 的 blocking #8 無法修,結局定義與整份報告語言都連動。
  - 診斷過程與副產品(JM 距離方法論教訓、正典三折從未存過 pixel-exact 預測等)已寫入 vault [[20260805_巨型場景診斷與P4操作點實驗]]。
...
- **下一步(2026-08-05 使用者裁決後更新)**:
  ...
  3. **(已收斂)** 2026-08-05 09:40 啟動的 mask / prediction artifact JSON RLE 序列化 schema 共識案已收斂,其輸出格式已被 P4 操作點實驗直接採用(見上「執行中程序」)。
  4. **(新增,阻擋中)** P4 prereg 凍結卡在 🔴 使用者裁決(pooled vs S 評成敗)+ codex 8 條 BLOCKING 修正,兩者皆待處理,詳見上「執行中程序」與 institution 正本第 9 條。

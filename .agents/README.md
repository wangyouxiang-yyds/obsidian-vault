...
- `skills/` — 兩個 harness 共用的 skill 規格（目前：ingest-paper, critical-read, lit-corpus-analysis, paper-triage）
  - Claude Code 實際讀取位置：`.claude/skills/<name>/SKILL.md`（2026-07-10 核對：與此處內容逐字相同）
  - `~/.claude/skills/ingest-paper/` 與 `~/.agents/skills/ingest-paper/`（個人層級，不隨 repo 走）也存在同一份 ingest-paper，
    2026-07-10 核對三者逐字相同；但沒有機制強制同步，之後若改 skill 內容，**先改這裡，再手動同步其他三處**。
...
- Codex 預設 model／reasoning effort：`~/.codex/config.toml` 沒有查到明確設定值，需要到 Codex 介面實測後回填，不要用猜的。

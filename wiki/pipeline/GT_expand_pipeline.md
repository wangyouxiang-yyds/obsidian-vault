...
> **2026-07-18 更新**：舊「雙 gate」已降為 legacy descriptive 指標，正式判定改用 **Evaluation Contract v1.0**（見下方新增小節），原因是 2026-07-16 溯源稽核發現 355-scene 不是統計獨立樣本。
...
- ~~**正式驗收採雙 gate**~~（2026-07-18 起降為 legacy descriptive，不再單獨作為判定依據）：
  1. `pooled_oil_iou > 0.362`
  2. per-scene 配對 Wilcoxon 檢定 vs baseline，p<0.05 且方向為正
- **Baseline 三折基準**（run `20260701-133158` / `20260701-204746` / `20260702-032032`）：pooled_oil_iou = 0.3695 / 0.3119 / 0.3157，mean = **0.332**（⚠️ 修復前產出，決定性正本重跑後另有數字，見 [[20260718_訓練不可重現根因與決定性修復]]）
- 注意：GT_expand 的 IoU 數字仍不可直接與 0422 的整圖 IoU 比較（分母不同）
- 與 0422 公平比較的完整協議尚未完成（`TODO/02_eval_protocol_comparison.md` 內容已過時，見第四節說明）

### 五-a：Evaluation Contract v1.0（2026-07-18 起唯一正式判定協定）

2026-07-16 溯源稽核發現全語料僅 12 個事件群，350/355 test scene 在同 fold train 中有同事件姊妹景（94% 近重複），355-scene 視為獨立樣本的假設失效。Evaluation Contract v1.0 因此改以 **事件群（12 群）** 為推論單位：

- **M1（主要）**：region（GoM/Atlantic/Other）× size（tertile）9-stratum 等權 macro Oil IoU
- **M2（次要）**：355 scenes 等權 per-scene mean Oil IoU
- **推論**：event-grouped cluster bootstrap（B=9999，從 12 個事件群有放回抽樣），CI 不含 0 才算顯著
- 舊 pooled_oil_iou、355-scene Wilcoxon 全部降為描述性 legacy 指標

完整定義與 grouped replay 結果（tversky vs baseline 顯著、Prithvi+UPerNet vs tversky 顯著、單純換骨幹 Prithvi+DeepLabV3 打平）見 [[20260716_資料溯源洩漏與評估合約v1]]。
...
- [[20260716_資料溯源洩漏與評估合約v1]] — 事件層級溯源洩漏稽核、Evaluation Contract v1.0（取代第五節舊雙 gate）、backbone 政策決策
- [[20260718_訓練不可重現根因與決定性修復]] — Albumentations RNG 未鎖導致訓練不可重現的根因與修復，第三-c節 tversky/cw31 數字需以此篇 caveat 解讀
- [[20260724_Focal_Tversky小圖實驗可行性評估]] — 下一個候選 loss 實驗的條件式規劃

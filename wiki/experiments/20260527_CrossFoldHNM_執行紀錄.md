...
## cfHNM 訓練結果（2026-05-30 完成）

| Fold | 結果目錄 | Best Epoch | val_mIoU | val_Oil_IoU | val_Bg_IoU |
|------|---------|-----------|----------|------------|-----------|
| 1 | `20260529-025248_..._fold1` | 28 | 0.5553 | 0.1640 | 0.9465 |
| 2 | `20260529-113954_..._fold2` | 30 | 0.5977 | 0.2595 | 0.9358 |
| 3 | `20260529-194052_..._fold3` | 63 | 0.6167 | 0.2493 | 0.9842 |
| **平均** | | | **0.5899** | **0.2243** | |
...
| 組別 | Split | 那 0.5 是什麼 | avg val_mIoU | avg val_Oil_IoU |
|------|-------|-------------|-------------|----------------|
| A | GB1.5 | 隨機背景 | 0.6011 | 0.2423 |
| B | GB1.0_cfhnm | cfHNM hard negative | 0.5899 | 0.2243 |

**結論：cfHNM（B 組）未能超越隨機背景（A 組）**，三個 fold 全部 A 組稍高。

### 可能原因分析

1. **早停過早**：cfHNM 三個 fold 分別在 epoch 28、30、63 停止（A 組為 19、50、76），HN 特徵尚未充分收斂。patience=50 對 HN 增強訓練可能偏緊。
2. **挖掘模型單一**：全部 220 場景使用同一個 fold3 best.pt 挖掘（val mIoU=50.3%），模型偏差造成 HN 分布不夠多樣。
3. **HN 比例不足**：HN_TO_OIL_RATIO=0.5 補入量可能不足以顯著影響決策邊界，需嘗試更高比例。
